	...
	if (rc != PR_CAP_NONE) {  /* new proxy */
		...
		if (curproxy->cap & PR_CAP_BE) {
			...
			curproxy->url_param_len = defproxy.url_param_len;
                        ...
                        curproxy->body_param_patterns = defproxy.body_param_patterns;
...
stats_error_parsing:
		...
		if (!*args[2]) {
			...
			else if (!strcmp(args[2], "crc16")) {
				curproxy->lbprm.algo |= BE_LB_HFCN_CRC16;
			}
			else if (!strcmp(args[2], "crc32")) {
				curproxy->lbprm.algo |= BE_LB_HFCN_CRC32;
			}
			else {
				Alert("parsing [%s:%d] : '%s' only supports 'sdbm', 'djb2', 'crc16', 'crc32', or 'wt6' hash functions.\n", file, linenum, args[0]);
				err_code |= ERR_ALERT | ERR_FATAL;
				goto out;
			}
